...
layui.use(['layer', 'form', 'admin', 'laydate', 'ax','upload'], function () {
    ...
    //获取用户信息
    var ajax = new $ax(Feng.ctxPath + "/news/getNewsInfo?newId=" + Feng.getUrlParam("newId"));
    var result = ajax.start();
    editor.txt.html(result.data.newContent);
    $("#newTags").tagsInput().importTags(result.data.newTags);
    $("#newKeyword").tagsInput().importTags(result.data.newKeyword);
    $('#img1').attr('src', '../'+result.data.newImgUrl);
    newImgUrl = result.data.newImgUrl;
