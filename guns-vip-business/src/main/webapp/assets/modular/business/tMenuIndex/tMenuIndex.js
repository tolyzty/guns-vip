layui.use(['layer', 'form', 'ztree', 'laydate', 'admin', 'ax', 'table', 'treetable'], function () {
    ...
    Menu.openAddMenu = function () {
        ...
        top.layui.admin.open({
            type: 2,
            title: '添加菜单',
            content: Feng.ctxPath + '/tMenuIndex/tMenuIndex_add',
            end: function () {
    ...
    Menu.onEditMenu = function (data) {
        ...
        top.layui.admin.open({
            type: 2,
            title: '编辑菜单',
            content: Feng.ctxPath + '/tMenuIndex/tMenuIndex_edit?menuId=' + data.menuId,
            end: function () {
                admin.getTempData('formOk') && Menu.initTable(Menu.tableId);
            }
        });
    ...
    Menu.onDeleteMenu = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/tMenuIndex/remove", function () {
                Feng.success("删除成功!");
    ...
    Menu.initTable = function (menuId, data) {
        return treetable.render({
            elem: '#' + menuId,
            url: Feng.ctxPath + '/tMenuIndex/listTree',
            where: data,
            page: false,
            height: "full-98",
            cellMinWidth: 100,
    ...
    //初始化左侧部门树
    var ztree = new $ZTree("menuTree", "/tMenuIndex/selectMenuTreeList");
