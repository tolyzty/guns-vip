...
import cn.stylefeng.guns.modular.system.mapper.TmenuIndexMapper;
 ...
 */
@Service
public class TmenuIndexService extends ServiceImpl<TmenuIndexMapper, TmenuIndex> {

    @Autowired
    private TmenuIndexMapper menuMapper;
     ...
     */
    @Transactional
    public void addMenu(TmenuIndexDto menuDto) {
        ...
        //组装属性，设置父级菜单编号
        TmenuIndex resultMenu = this.menuSetPcode(menuDto);
    ...
    public void updateMenu(TmenuIndexDto menuDto) {
        ...
        TmenuIndex menu = this.getById(id);
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void updateSubMenuLevels(TmenuIndex oldMenu, TmenuIndex newMenu) {

        List<TmenuIndex> menus = menuMapper.getMenusLikePcodes(oldMenu.getCode());

        for (TmenuIndex menu : menus) {
    ...
    public void delMenuContainSubMenus(Long menuId) {
        ...
        TmenuIndex menu = menuMapper.selectById(menuId);

        //删除当前菜单
        delMenu(menuId);

        //删除所有子菜单
        List<TmenuIndex> menus = menuMapper.getMenusLikePcodes(menu.getCode());

        for (TmenuIndex temp : menus) {
    ...
    public Page<Map<String, Object>> selectMenus(String condition, String level, Long menuId) {
        ...
        if (menuId != null && menuId != 0L) {
            TmenuIndex menu = this.getById(menuId);
            code = menu.getCode();
        }
     ...
     */
    public TmenuIndex selectByCode(String code) {
     ...
     */
    public TmenuIndex menuSetPcode(TmenuIndexDto menuParam) {

        TmenuIndex resultMenu = new TmenuIndex();
        ...
        if (ToolUtil.isEmpty(menuParam.getPid()) || menuParam.getPid().equals(0L)) {
            ...
            TmenuIndex pMenu = this.getById(pid);
